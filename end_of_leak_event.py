...
def test_stop_inject_and_time_wait_event():
    try:
        global injection_process
        tests.stop_inject_water(injection_process)
        sleep(TIME_TO_END_EVENT)
    except Exception as e:
        pytest.fail(str(e))


def test_restart_inject_and_leak_ends():
    global injection_process
    try:
        injection_process = tests.start_inject_water()
        tests.connect_to_product(driver, site_number, water_system_number)
        end_event_gotten = tests.is_command_was_gotten_loop(driver, site_number, water_system_number, product_id,
...
def test_new_leak_event_starts():
    test_warning_message_arrived()
